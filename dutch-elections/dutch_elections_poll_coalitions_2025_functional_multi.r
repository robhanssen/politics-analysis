...
TIME_SLICE <- 4 # nolint; highly memory intensive!
...
coalitions_time <- lapply(election$election[seq_len(TIME_SLICE)], generate_coalitions)
...
x <- map_df(coal_probs, \(x) x, .id = "date") %>% bind_rows() %>% pivot_longer(!date, names_to = "party", values_to = "prob") 
...
x %>%
    ...
    ggrepel::geom_text_repel(data = x %>% filter(date == first(x)$date), aes(x = 0.7, y = prob, label=party), hjust = 1) +
    # scale_x_discrete(transform = "rev") +
    labs(
        title = "Waarschijnlijkheid dat de PVV in een coalitie zit",
        y = NULL,
        x = NULL,
    ) +
